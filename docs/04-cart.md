...
> `price` per item is locked at the time it is added to the cart. If the product price changes afterward, the cart price will not update until checkout — at which point a `409` is returned and the cart is refreshed. See [Orders](./05-orders.md) for details.
